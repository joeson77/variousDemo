...
public class Demo {
	...
	private static final String STORE_DIR = "/Users/liyunjian/Downloads/文件/jump_screencapture";
	...
	private final String[] ADB_SCREEN_CAPTURE_CMDS = { "/usr/local/bin/adb shell screencap -p /sdcard/" + IMAGE_NAME,
			"/usr/local/bin/adb pull /sdcard/current.png " + STORE_DIR };
	...
	// 棋子的宽度，从截屏中量取，自行调节
	private final int halmaBodyWidth = 77;
	...
	private void executeCommand(String command) {
		//ProcessBuilder pb = new ProcessBuilder("/Users/David/Applications/android-sdk-macosx/platform-tools/adb", command);
	...
	private void doJump(double distance) {
		...
		// 执行按压操作
		String command = String.format("/usr/local/bin/adb shell input swipe %s %s %s %s %s", swipeX, swipeY, swipeX, swipeY,
				pressTime);
	...
	private void replayGame() {
		String command = String.format("/usr/local/bin/adb shell input tap %s %s", swipeX, swipeY);
		executeCommand(command);
	}
